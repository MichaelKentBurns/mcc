...
extern ptr     ustrNewPool  __ARGS((int nquick, long baseid, ptr (*quicktab)[]));

/* add an optional top level index to the pool */
extern int     ustrAddIndex __ARGS((ptr pool));               

/* print pool statistics to file */
extern void    ustrPoolStats __ARGS((ptr pool, ptr fd));               

/* free a ustr pool , calling user function elfree for each element */
extern int   ustrFreePool __ARGS((ptr pool, ptr node, void (*elfree)())); 
